...
spark = SparkSession \
    ...
    .appName(f"Query 3 (Dataframe API) (Task 7)") \
    .getOrCreate()
...
joined_df = rev_geo_df.join(crime_df_filtered.hint("broadcast"),['LAT','LON']) \
                 .select(crime_df_filtered['Vict Descent'], rev_geo_df['Zip Code']) \
                 .join(income_df.hint("merge"), 'Zip Code') 
...
query_3_result_1 = joined_df.join(max_zips.hint("shuffle_hash"),'Zip Code') \
...
query_3_result_1.explain(mode = "formatted")

#keep crimes related only to 3 min zip codes
query_3_result_2 = joined_df.join(min_zips.hint("shuffle_replicate_nl"),'Zip Code') \
                          .groupby('Vict Descent').agg(count('*').alias('#Victims')) \
                          .orderBy(desc('#Victims'))

#show results, time taken and schema
...
query_3_result_2.explain(mode = "formatted")
